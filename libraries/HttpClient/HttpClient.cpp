...
int HttpClient::startRequest(const char* aServerName, uint16_t aServerPort, const char* aURLPath, 
    const char* aHttpMethod, const char* aUserAgent)
{
    beginRequest(); // 20140414 added by yehnan

    const tHttpState initialState = iState;
...
int HttpClient::startRequest(const IPAddress& aServerAddress, const char* aServerName, uint16_t aServerPort, 
    const char* aURLPath, const char* aHttpMethod, const char* aUserAgent)
{
    beginRequest(); // 20140414 added by yehnan
...
{
    sendHeader(HTTP_HEADER_CONNECTION, "close"); // 20140414 added by yehnan
    
    iClient->println(); // an empty line seperating headers and body
    iState = eRequestSent;
}
...
void HttpClient::endRequest()
{
    if(iState < eRequestSent){
        finishHeaders(); // We still need to finish off the headers
    }
...
int HttpClient::skipResponseHeaders()
{
    ...
    if(endOfHeadersReached()){ // Success
        return HTTP_SUCCESS;
    }
    else{ // We must've timed out
        return HTTP_ERROR_TIMED_OUT;
    }
}
...
int HttpClient::readHeader()
{
    ...
    switch(iState){
      ...
      case eLineStartingCRFound:
        ...
        break;
      default: // We're just waiting for the end of the line now
        break;
    }
