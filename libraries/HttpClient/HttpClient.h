...
// to be called.  Usually indicates your code is using the class incorrectly
...
// Definitions of some common methods and headers
...
// FIXME Update tempToPachube example to calculate Content-Length correctly
    ...
    */
    ...
    virtual size_t write(uint8_t aByte) 
      { if (iState < eRequestSent) { finishHeaders(); }; return iClient-> write(aByte); };
    virtual size_t write(const uint8_t *aBuffer, size_t aSize) 
      { if (iState < eRequestSent) { finishHeaders(); }; return iClient->write(aBuffer, aSize); };
